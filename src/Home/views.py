...
def update_product(request, pk):
    if request.user == get_object_or_404(Product, pk=pk).author:
        ...
        return HttpResponse(
            " <h1> You are not allowed to edit this product </h1>", status=403
        )
...
# create order to admin with checkout button

def create_order(request):
    if request.user.is_authenticated:
        cart = Cart.objects.get(user=request.user)
        cart.products.clear()
        return render(request, 'Home/create_order.html', {'cart': cart})
    else:
        return render(request, 'Home/404.html', {'status': 404})
